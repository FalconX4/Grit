...
func on_item_slot_drag_failed(sender: ItemSlotGrid, last_mouse_position: Vector2, slot_dragged: ItemSlot, _is_mouse_right_drag: bool):
	for grid in gridList:
		if grid != sender and grid.is_processing_input():
			if grid.on_item_slot_dragged(last_mouse_position, slot_dragged) != null:
				break 
