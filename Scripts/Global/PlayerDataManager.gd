...
signal player_data_version_changed	# Sent with old version as argument
...
func _init() -> void:
	if FileAccess.file_exists(DATA_FILE_PATH):
		...
		if not _data_loaded:
			push_error("Failed to load player data file: %s" % DATA_FILE_PATH)
		else:
			version = get_setting(SECTION, VERSION_KEY, VERSION)
			if version != VERSION:
				player_data_version_changed.emit(version)
